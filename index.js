...
const CONTENTS = [
    ...
    },
    {
        contentType: "image",
        themes: {
            christmas: "https://arbetsformedlingen.se/webdav/files/logo/logo.svg",
...
function getRandomContent() {
    ...
    const totalChance = CONTENTS.reduce((sum, { chance }) => sum + chance, 0);
...
const displayContent = (contentObject, container) => {
    ...
    if (contentObject.contentType === 'emoji' || contentObject.contentType === 'text') {
        // Handle emoji or text content
...
const createUI = (bitSet, phraseBitSet) => {
    for (let i = 0; i < TOTAL_TILES; i++) {
        ...
        if (isSet(bitSet, i)) {
            bitElement.innerHTML = getRandomContent(); // Set to random emoji if bit is active
        ...
        bitElement.addEventListener('click', () => {
            ...
            if (isSet(bitSet, index)) {
                ...
                //bitElement.innerHTML = getRandomContent(); // Show random emoji when activated
            }
...
const initializeGame = () => {
    ...
    if (!phraseBitSet) {
        ...
        // Step 1: Create an array of phrase indices (0 to phrases.length - 1)
        const phraseIndices = Array.from({ length: phrases.length }, (_, index) => index);

        // Step 2: Shuffle the phrase indices
        const shuffledPhraseIndices = phraseIndices.sort(() => Math.random() - 0.5);
        // Step 3: Assign the shuffled indices to the tiles
        for (let i = 0; i < TOTAL_TILES; i++) {
